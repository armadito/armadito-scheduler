package Armadito::Scheduler::Logger;
...
sub new {
	...
	foreach ( $params{backends} ? @{ $params{backends} } : 'Stderr' ) {
		...
		my $package = "Armadito::Scheduler::Logger::$backend";
		$package->require();
		if ($EVAL_ERROR) {
	...
	$self->debug($Armadito::Scheduler::VERSION_STRING);
	return $self;
}
...
Armadito::Scheduler::Logger - Armadito Scheduler logger
...
the scheduler configuration object, to be passed to backends
